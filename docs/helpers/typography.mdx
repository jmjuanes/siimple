...
Typography helpers can be customized using the `fonts`, `fontSizes`, `fontWeights` and `lineHeights` fields of your theme configuration.

```js title=siimple.config.js
export default {
    fontSizes: ["0.875rem", "16px", "1.25rem", "1.5rem", "2rem", "2.5rem", "3rem", "3.5rem"],
    // ...other configuration
};
```
...
For each size defined in the `lineHeights` field of your theme, there will be a helper with the pattern `has-lh-{name}` to apply the specified line-height.

The following line-height helpers will be always included, regardless the values you provide in your theme configuration:
