...
**Siimple** is based in a set of configuration rules written in JavaScript, where you can define your theme, the core modules that you need to include and additional styles.
...
- `useRebootStyles`: includes the styles defined in the [reboot module](/reboot). Default is `true`.
- `useMarkupStyles`: include the styles defined in the [markup module](/markup). Default is `true`.
- `useElements`: include elements. Default is `true`.
- `useUtilities`: include utilities. Default is `true`.
- `useIcons`: include icons. Default is `true`.
