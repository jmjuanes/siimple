...
@include siimple.configure(
    ...
    $theme: (
        ...
        "colors": (
            ...
            "body": siimple.get-color("gray", 800),
        ),
        "fonts": (
...
Custom body styles can be disabled adding a `"use-custom-body": false` flag in the `$flags` section of the configuration. In this case, the default body styles will be applied.
...
| Flag | Description |
|------|-------------|
| `"use-custom-body"` | Enable or disable applying custom styles to the `<body>` element defined in the `$body` section. Default is `true`. |
| `"use-custom-styles"` | Enable or disable custom styles defined in the `$styles` section. Default is `true`. |
| `"use-components"` | Enable or disable core components. Default is `true`. |
...
@include siimple.configure(
    $flags: (
        "use-custom-body": true,
        "use-custom-styles": true,
        "use-components": true,
    ),
);
```
