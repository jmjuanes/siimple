...
- `useReboot`: include our reboot styles. Default is `true`.
- `useElements`: include elements. Default is `true`.
- `useHelpers`: include helpers styles. Default is `true`.
- `useMarkup`: include our markup styles. Default is `true`.
- `useIcons`: include our icons styles. Default is `true`.
...
```js title=siimple.config.js
import myPreset from "my-presets-lib";
